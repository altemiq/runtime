...
public static partial class VectorD
{
    ...
    extension(Vector3D value)
    {
        ...
        /// <returns>The input converted to a new <see cref="Vector4D" /> with the new elements zeroed.</returns>
        ...
        /// <returns>The input converted to a new <see cref="Vector4D" /> with the new elements undefined.</returns>
        ...
        public double GetElement(int index) =>
            (uint)index >= Vector3D.ElementCount
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Vector3D WithElement(int index, double value1) =>
            (uint)index >= Vector3D.ElementCount
                ? throw new ArgumentOutOfRangeException(nameof(index))
                : value.AsVector256Unsafe().WithElement(index, value1).AsVector3D();
    }
}