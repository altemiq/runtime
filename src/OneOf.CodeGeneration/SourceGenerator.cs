...
";
    ...
    /// <inheritdoc/>
    public void Initialize(IncrementalGeneratorInitializationContext context) => context.RegisterPostInitializationOutput(static ctx =>
    {
        const int Count = 10;

        for (var i = 0; i < Count; i++)
        {
            Add(GenerateOneOfGeneric(i + 1), ref ctx);
        }

        Add(GenerateOneOf(Count), ref ctx);

        static void Add((string Name, MemberDeclarationSyntax Declaration) member, ref IncrementalGeneratorPostInitializationContext context)
        {
