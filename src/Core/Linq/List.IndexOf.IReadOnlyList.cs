// -----------------------------------------------------------------------
// <copyright file="List.IndexOf.IReadOnlyList.cs" company="Altemiq">
...
/// <content>
/// <see cref="IReadOnlyList{T}"/> extensions for getting the index.
/// </content>
public static partial class List
{
    ...
    extension<T>(IReadOnlyList<T>? buffer)
        where T : IEquatable<T>
    {
        ...
        public int IndexOf(IReadOnlyList<T> value) => buffer?.IndexOf(value, 0) ?? -1;
        ...
        public int IndexOf(IReadOnlyList<T> value, int startIndex) => buffer is not null && buffer.Count > startIndex ? buffer.IndexOf(value, startIndex, buffer.Count - startIndex) : -1;
        ...
        /// <returns>The zero-based index position of <paramref name="value"/> from the start of this instance if found, or -1 if it is not. If <paramref name="value"/> is an empty list, the return value is <paramref name="startIndex"/>.</returns>
        public int IndexOf(IReadOnlyList<T> value, int startIndex, int count)
        {
        ...
        /// <param name="startIndex">The search starting position.</param>
        /// <returns>The zero-based index position of <paramref name="value"/> from the start of this instance if found, or -1 if it is not.</returns>
        public int IndexOf(T value, int startIndex) => buffer is not null && buffer.Count > startIndex ? buffer.IndexOf(value, startIndex, buffer.Count - startIndex) : -1;
