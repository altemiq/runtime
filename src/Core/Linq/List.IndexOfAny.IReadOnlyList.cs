...
public static partial class List
{
        ...
        where T : IEquatable<T>
    {
        ...
        public int IndexOfAny(params IReadOnlyList<T>[] anyOf) => buffer.IndexOfAny(0, anyOf);
        ...
        /// <param name="anyOf">The array of <typeparamref name="T"/> sequences to seek.</param>
        /// <returns>The zero-based index position of the first occurrence in this instance where any <typeparamref name="T"/> sequence in <paramref name="anyOf"/> was found; -1 if no byte sequence in <paramref name="anyOf"/> was found.</returns>
        public int IndexOfAny(int startIndex, params IReadOnlyList<T>[] anyOf) => buffer is not null && buffer.Count > startIndex ? buffer.IndexOfAny(startIndex, buffer.Count - startIndex, anyOf) : -1;
        ...
        /// <param name="anyOf">The array of <typeparamref name="T"/> sequences to seek.</param>
        /// <returns>The zero-based index position of the first occurrence in this instance where any <typeparamref name="T"/> sequence in <paramref name="anyOf"/> was found; -1 if no byte sequence in <paramref name="anyOf"/> was found.</returns>
        public int IndexOfAny(int startIndex, int count, params IReadOnlyList<T>[] anyOf)
        {
