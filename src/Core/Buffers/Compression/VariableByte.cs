    ...
    private static void HeadlessCompress(int[] source, ref int sourceIndex, int[] destination, ref int destinationIndex, int length)
    {
        ...
        for (var k = sourceIndex; k < sourceIndex + length; k++)
        {
            ...
            else if (val < (1 << 14))
            {
                ...
                buffer.WriteSByte((sbyte)(Extract7BitsMaskless(1, val) | (1 << 7)));
            }
            else if (val < (1 << 21))
            {
                buffer.WriteSByte(Extract7Bits(0, val));
                buffer.WriteSByte(Extract7Bits(1, val));
                buffer.WriteSByte((sbyte)(Extract7BitsMaskless(2, val) | (1 << 7)));
            }
            else if (val < (1 << 28))
            {
                buffer.WriteSByte(Extract7Bits(0, val));
                ...
                buffer.WriteSByte((sbyte)(Extract7BitsMaskless(3, val) | (1 << 7)));
            }
            else
            {
                buffer.WriteSByte(Extract7Bits(0, val));
                ...
                buffer.WriteSByte((sbyte)(Extract7BitsMaskless(4, val) | (1 << 7)));
            }
        }
